...
/mob/living/carbon/human/blob_act()
	...
	var/dam_zone = pick("chest", "groin", "head", "l_arm", "r_arm", "l_leg", "r_leg")
	var/obj/item/organ/external/affecting = get_organ(ran_zone(dam_zone))
	apply_damage(rand(20,40), BRUTE, affecting, run_armor_check(affecting, "melee"))
	return
					...
					for (var/datum/data/record/R in data_core.security)
						if (R.fields["id"] == E.fields["id"])
							if(hasHUD(usr,"security"))
								...
								if(istype(usr,/mob/living/carbon/human))
									var/mob/living/carbon/human/U = usr
									R.fields[text("com_[counter]")] = text("Made by [U.get_authentification_name()] ([U.get_assignment()]) on [ss13time2text()]<BR>[t1]")
								if(istype(usr,/mob/living/silicon/robot))
									var/mob/living/silicon/robot/U = usr
									R.fields[text("com_[counter]")] = text("Made by [U.name] ([U.modtype] [U.braintype]) on [ss13time2text()]<BR>[t1]")
