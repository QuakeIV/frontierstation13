					...
					spawn(16)
						...
						for(var/M in being_built.materials)
							switch(M)
								...
								if("$plasma")
									linked_lathe.plasma_amount = max(0, (linked_lathe.plasma_amount-being_built.materials[M]*linked_lathe.mat_efficiency))
								if("$uranium")
...
/obj/machinery/computer/rdconsole/attack_hand(mob/user as mob)
	...
	switch(screen)
				...
				for(var/M in D.materials)
					...
					if(copytext(M, 1, 2) == "$")
						switch(M)
							...
							if("$plasma")
								if(D.materials[M]*linked_lathe.mat_efficiency > linked_lathe.plasma_amount) check_materials = 0
							if("$uranium")
		...
		if(3.2) //Protolathe Material Storage Sub-menu
			...
			dat += "<UL>"
			for(var/M in list(DEFAULT_WALL_MATERIAL, "glass", "gold", "silver", "plasma", "uranium", "diamond"))
				var/amount
				var/sheetsize = 2000
				switch(M)
					if(DEFAULT_WALL_MATERIAL)
